...
/** Pedidos pendentes recebidos (pra aceitar/recusar) e enviados (aguardando resposta) pelo jogador atual. */
export function useFriendRequests(): { incoming: FriendRequest[]; outgoing: FriendRequest[] } {
  return useAppStore(
    useShallow((s) => {
      const pending = s.friendships.filter((f) => f.status === 'pending');
      const incoming = pending
        .filter((f) => f.addresseeId === s.currentPlayerId)
        .map((f) => ({ friendship: f, player: s.players.find((p) => p.id === f.requesterId) }))
        .filter((r): r is FriendRequest => !!r.player);
      const outgoing = pending
        .filter((f) => f.requesterId === s.currentPlayerId)
        .map((f) => ({ friendship: f, player: s.players.find((p) => p.id === f.addresseeId) }))
        .filter((r): r is FriendRequest => !!r.player);
